...
bool Estimator::visualInitialAlign() {
  ...
  // solve scale
  bool result = VisualIMUAlignment(all_image_frame, Bgs, g, x);  // g是在C0坐标系下
  if (!result) {
  ...
  std::cout << "[cggos " << __FUNCTION__ << "] Bgs[0]: " << Bgs[0].transpose() << std::endl;
  std::cout << "[cggos " << __FUNCTION__ << "] s: " << s << std::endl;
  std::cout << "[cggos " << __FUNCTION__ << "] G (param): " << G.transpose() << std::endl;
  std::cout << "[cggos " << __FUNCTION__ << "] g in world: " << g.transpose() << std::endl;
  std::cout << "[cggos " << __FUNCTION__ << "] Rs[0] ypr: " << Utility::R2ypr(Rs[0]).transpose() << std::endl;
...
void Estimator::double2vector() {
  ...
  if (abs(abs(origin_R0.y()) - 90) < 1.0 || abs(abs(origin_R00.y()) - 90) < 1.0) {
    ...
    rot_diff =
        Rs[0] *
        Quaterniond(para_Pose[0][6], para_Pose[0][3], para_Pose[0][4], para_Pose[0][5]).toRotationMatrix().transpose();
  }

  for (int i = 0; i <= WINDOW_SIZE; i++) {
    Rs[i] =
        rot_diff *
        Quaterniond(para_Pose[i][6], para_Pose[i][3], para_Pose[i][4], para_Pose[i][5]).normalized().toRotationMatrix();
  ...
  for (int i = 0; i < NUM_OF_CAM; i++) {
    ...
    ric[i] =
        Quaterniond(para_Ex_Pose[i][6], para_Ex_Pose[i][3], para_Ex_Pose[i][4], para_Ex_Pose[i][5]).toRotationMatrix();
  }
  ...
  if (relocalization_info) {
    ...
    Vector3d relo_t;
    relo_r =
        rot_diff * Quaterniond(relo_Pose[6], relo_Pose[3], relo_Pose[4], relo_Pose[5]).normalized().toRotationMatrix();
...
#pragma endregion
   ...
   *
   * MARGIN_OLD:
   * 如果次新帧是关键帧，则丢弃滑动窗口内最老的图像帧，同时对与该图像帧关联的约束项进行边缘化处理。
...
#pragma region  // Marginalization MARGIN_OLD
    ...
    // 添加第0帧和第1帧之间的IMU预积分值以及第0帧和第1帧相关优化变量
    if (pre_integrations[1]->sum_dt < 10.0) {
      auto *imu_factor = new IMUFactor(pre_integrations[1]);
      auto *residual_block_info = new ResidualBlockInfo(
          imu_factor, nullptr,
          vector<double *>{para_Pose[0], para_SpeedBias[0], para_Pose[1], para_SpeedBias[1]},  // 7, 9, 7, 9
          vector<int>{0, 1});
      marginalization_info->addResidualBlockInfo(residual_block_info);
    }
      ...
      for (auto &it_per_id : f_manager.feature) {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame) {
          ...
          if (ESTIMATE_TD) {
            ...
            auto *residual_block_info =
                new ResidualBlockInfo(f_td, loss_function,
                                      vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0],
                                                       para_Feature[feature_index], para_Td[0]},  // 7, 7, 7, 1, 1
                                      vector<int>{0, 3});
            ...
            // 不仅仅会边缘化第一帧相关的优化变量，还会边缘化掉以第一帧为起始观察帧的路标点
            auto *residual_block_info =
                new ResidualBlockInfo(f, loss_function,
                                      vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0],
                                                       para_Feature[feature_index]},  // 7, 7, 7, 1
                                      vector<int>{0, 3});
    ...
    // 滑窗预移动
    // 将滑窗里关键帧位姿移位，这里是保存了所有状态量的信息，为什么没有保存逆深度的状态量呢
...
void Estimator::slideWindowOld() {
  ...
  if (shift_depth) {
    ...
    P1 = Ps[0] + Rs[0] * tic[0];
    f_manager.removeBackShiftDepth(R0, P0, R1, P1);  // 把老帧的信息传给新帧
  } else
